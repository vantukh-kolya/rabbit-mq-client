...
use Throwable;
use VantukhKolya\RabbitMqClient\Contracts\RabbitMQQueueContract;
...
class RabbitMQQueue extends Queue implements QueueContract, RabbitMQQueueContract
{
    ...
    {
        return Arr::get($options, 'exchange_routing_key') ?? ltrim(
            sprintf($this->getConfig()->getExchangeRoutingKey(), $destination),
            '.'
        );
    }
